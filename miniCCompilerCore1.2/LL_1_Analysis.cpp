		...
		while (loop)
		{
			...
			if (g.isNonterminator((*it_r).data))//是否为非终结符
			{
				...
				if (existATItemInATItemList((*it_f_t).mFirst, (string)"$"))
				{
					...
					{
						if (!existATItemInATItemList((*it_f).mFirst, (string)"$"))//不存在
						{
	...
	while (true)
	{
		if (!analysisStack.empty())	//stack不为空
		{
			...
			if (g.isTerminator(X.type))	//是终结符
			{
				...
				{
					LL_1_AnalysisItem item(analysisResult.size() + 1, analysisStackData, list_left, a.data + "匹配");
			...
			else
			{
				if (X.type == Delimiter_Pound_Sign)	//是否 "#"
				{
					if (X.type == a.type)
					{	//分析结束
						LL_1_AnalysisItem item(analysisResult.size() + 1, analysisStackData, list_left, (string)"接受");
						analysisResult.push_back(item);
						//cout << "接受" << endl;
						return true;
					}
